...
export default function DaeunPage() {
  ...
  if (stage === 'result') return <ReadingResult title={`${form.name}님의 대운 해설`} subtitle={`${calType === 'solar' ? '양력' : '음력'} · 앞으로의 10년`} character="sinryeong" sections={SECTIONS.map(s=>({id:s.key,title:s.title,body:String(result[s.key as keyof DaeunResult]||'')}))} onBack={()=>setStage('input')}/>
      ...
      <div className="max-w-md mx-auto px-4 pt-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-gray-500 text-xs mt-0.5">무등산 신령님의 10년 대운 분석 · 이용권 준비 중</p>
          </div>
        </div>
