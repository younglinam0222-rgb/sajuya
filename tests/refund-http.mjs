...
const server=spawn(process.execPath,['node_modules/next/dist/bin/next','start','-H','127.0.0.1','-p','3114'],{env:{...process.env,NEXTAUTH_URL:origin,NEXTAUTH_SECRET:'refund-http-test',NEXT_PUBLIC_SUPABASE_URL:'http://127.0.0.1:3110',NEXT_PUBLIC_SUPABASE_ANON_KEY:'test',SUPABASE_SERVICE_ROLE_KEY:'test',ADMIN_USER_IDS:'owner',REFUNDS_ENABLED:'true',TOSS_SECRET_KEY:'',CRON_SECRET:'refund-worker-test'},stdio:['ignore','pipe','pipe']})
let logs='';server.stdout.on('data',b=>logs+=b);server.stderr.on('data',b=>logs+=b)
try{
 let ready=false;for(let i=0;i<120;i++){try{const r=await fetch(origin+'/api/refunds');if(r.status===401){ready=true;break}}catch{}if(server.exitCode!==null)throw Error(logs);await new Promise(resolve=>setTimeout(resolve,150))}assert.ok(ready,'test server ready')
 ...
 assert.equal(calls.length,0)
 assert.equal((await post('/api/refunds',payload)).status,200)
...
}finally{server.kill('SIGTERM');await new Promise(resolve=>server.once('exit',resolve));await new Promise(resolve=>fake.close(resolve))}
