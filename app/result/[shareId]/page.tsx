          ...
          {paidTitles.length > 0 && (
            <>
              <div className="flex items-center justify-between mb-2">
                <div className="text-xs font-bold text-[#555]">🔒 잠긴 판결 {paidTitles.length}개</div>
                <Link href="/saju" className="text-xs px-3 py-1.5 rounded-full font-bold text-white" style={{ background: charColor }}>
                  990원으로 전체 열기
                </Link>
              </div>
              <div className="space-y-2 mb-4">
                {paidTitles.map(t => (
                  <div key={t.id} className="rounded-2xl p-4 border border-gray-800 bg-[#111]">
                    <div className="flex items-start gap-3">
                      <div className="w-8 h-8 rounded-full flex items-center justify-center bg-gray-800 flex-shrink-0">🔒</div>
                      <div>
