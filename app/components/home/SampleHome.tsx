...
export default function SampleHome(){
 ...
 const playing=!paused&&!reduced&&!hovered&&!focusPaused&&visible&&!login&&!shop
 ...
 const charge=()=>session?setShop(true):openLogin()
 return <><div className="direction direction-b">
  <header className="sg-header"><Link href="/" className="sg-brand" aria-label="사주궁 홈">사주궁<span>四柱宮</span></Link><div className="sg-header-actions"><Link className="sg-reviews-link" href="/reviews">후기 게시판</Link>{session?<button onClick={charge}>{balance}냥 · 충전</button>:<button onClick={()=>openLogin()}>로그인</button>}</div></header>
  <main className="sg-main">
   ...
   {session&&<aside className="jr-return"><span>내가 받은 이야기</span><h2>저장한 풀이를 이어 읽어보세요</h2><p>완성된 결과는 보관함에서 다시 확인할 수 있어요.</p><Link href="/storage">내 보관함 보기 →</Link></aside>}<div className="cv-value-strip"><b>한 번의 사주 풀이, 이런 이야기까지.</b><span>12개 해석 · 주의할 점 · 인생 전략 · 선택질문</span><small>입력한 사주 정보를 바탕으로 AI가 작성하는 해석</small></div>
   ...
   <section className="sg-section sg-menus"><div className="sg-heading"><h2>내 고민에 맞는 운세</h2><button onClick={charge}>엽전 충전<Icon name="coin" size={17}/></button></div><p className="sg-group">프리미엄</p><div className="sg-paid">{MENUS.filter(m=>m.paid).map((m,i)=><Link key={m.href} href={m.href} className="sg-menu"><span className="sg-menu-number">0{i+1}</span><div><h3>{m.label}</h3><p>{m.desc}</p></div><span className="sg-menu-price">{m.href==='/gunghap'?'준비 중':<>{NYANG_PRICE.toLocaleString()}원 <small>1냥</small></>}</span><Icon name="arrow" size={21}/></Link>)}</div><p className="sg-group">그 밖의 운세</p><div className="sg-free">{MENUS.filter(m=>!m.paid).map(m=><Link key={m.href} href={m.href} className="sg-menu"><Icon name={icons[m.href]} size={26}/><div><h3>{m.label}</h3><p>{m.desc}</p></div><span className="sg-menu-price">{m.badge}</span></Link>)}</div><ProductConditions/></section>
   <ResultProof/>
   <TrustNote/><ServiceFAQ/>
   {!session&&<section className="sg-welcome"><p>처음 만나는 사주궁</p><h2>처음이라면<br/>일일운세 1회 무료</h2><span>가입 계정당 한 번, 부담 없이 만나보세요.</span><button className="sg-primary" onClick={()=>openLogin("/daily")}>첫 무료 운세 시작하기<Icon name="arrow" size={20}/></button></section>}
 ...
 </div>{shop&&<YeopjeunShop onClose={()=>setShop(false)} currentBalance={balance}/>}</>
}
