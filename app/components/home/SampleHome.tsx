...
import ReviewsPreview from '@/app/components/reviews/ReviewsPreview'
import HeaderAuthActions from './HeaderAuthActions'
import SiteShareBar from './SiteShareBar'
...
import {CHAR_IMG,CHARACTERS,BANNERS,MENUS} from './data'
...
export default function SampleHome(){
 ...
 const [paused,setPaused]=useState(false),[focusPaused,setFocusPaused]=useState(false),[visible,setVisible]=useState(true)
 const [loginTarget,setLoginTarget]=useState("/")
 const [cycle,setCycle]=useState(0),[reduced,setReduced]=useState(true),[hovered,setHovered]=useState(false)
 ...
 const {data:session,status}=useSession();const dialog=useRef<HTMLDialogElement>(null)
 const searchParams=useSearchParams();const router=useRouter();const pathname=usePathname()
 const chargeQuery=searchParams.get('open')==='charge'
 const showShop=shop||(status==='authenticated'&&chargeQuery)
 const playing=!paused&&!reduced&&!hovered&&!focusPaused&&visible&&!login&&!showShop
 ...
 const openLogin=(target="/")=>{setLoginTarget(target);setLogin(true)}
 const charge=()=>status==='authenticated'?setShop(true):openLogin('/?open=charge')
 const closeShop=()=>{
  setShop(false)
 ...
 return <><div className="direction direction-b">
  <header className="sg-header"><Link href="/" className="sg-brand" aria-label="사주궁 홈">사주궁<span>四柱宮</span></Link><div className="sg-header-actions"><Link className="sg-reviews-link" href="/reviews">후기 게시판</Link><HeaderAuthActions onLogin={()=>openLogin()} onCharge={charge}/></div></header>
  <main className="sg-main">
   <section data-autoplay-state={playing?'playing':reduced?'reduced':paused?'paused':focusPaused?'focus':hovered?'hover':!visible?'hidden':'dialog'} className="sg-hero" aria-label="추천 운세" aria-roledescription="캐러셀" onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)} onFocusCapture={e=>{if(!(e.target as HTMLElement).closest('.sg-playback')&&(e.target as HTMLElement).matches(':focus-visible'))setFocusPaused(true)}} onBlurCapture={e=>{if(!e.currentTarget.contains(e.relatedTarget as Node|null))setFocusPaused(false)}}>
    <div className="sg-slides" aria-live="off">{BANNERS.map(slide=>{const guide=CHARACTERS.find(c=>c.id===slide.charId)!;const active=slide.id===current;return <div className={`sg-hero-body ${active?'is-current':''}`} key={slide.id} aria-hidden={!active} inert={!active}>
     <div className="sg-hero-copy"><p className="sg-eyebrow">{guide.name}의 {guide.tag}</p><h1>{slide.title.split('\n').map(t=><span key={t}>{t}</span>)}</h1><p className="sg-hero-desc">{slide.desc}</p><Link href={slide.href} className="sg-primary">{slide.cta.replace(' →','')}<Icon name="arrow" size={20}/></Link><p className="sg-hero-price">{slide.href==='/saju'?`12개 해석 · ${formatNyang(1)} / 1,900원`:slide.href==='/gunghap'?'준비 중 · 현재 구매할 수 없어요':slide.href==='/daily'?'계정당 최초 1회 무료':'준비 중 · 현재 구매할 수 없어요'}</p><a className="cv-hero-example" href="#result-preview">결과 예시 먼저 보기</a></div>
    ...
    <div className="sg-autoplay-row"><span><b>{String(current+1).padStart(2,'0')} / {String(BANNERS.length).padStart(2,'0')}</b><span>{reduced?'동작 줄이기 설정 적용':paused?'자동 넘김 멈춤':!playing?'읽는 동안 잠시 멈춤':'6초마다 사진 자동 넘김'}</span></span><button disabled={reduced} className="sg-playback" aria-label={paused?'사진 자동 넘김 시작':'사진 자동 넘김 일시 정지'} onClick={()=>{setPaused(v=>!v);setFocusPaused(false)}}><span aria-hidden="true">{paused?'▷':'Ⅱ'}</span>{paused?'재생':'일시 정지'}</button></div>
   </section>
   {status==='authenticated'&&<aside className="jr-return"><span>내가 받은 이야기</span><h2>저장한 풀이를 이어 읽어보세요</h2><p>완성된 결과는 보관함에서 다시 확인할 수 있어요.</p><Link href="/storage">내 보관함 보기 →</Link></aside>}
   <SiteShareBar/>
   <div className="cv-value-strip"><b>한 번의 사주 풀이, 이런 이야기까지.</b><span>12개 해석 · 주의할 점 · 인생 전략 · 선택질문</span><small>입력한 사주 정보를 바탕으로 AI가 작성하는 해석</small></div>
   <Link className="sg-daily" href="/daily"><Icon name="sun" size={28}/><div><h2>일일운세 첫 1회 무료</h2><p>계정당 최초 1회 · 결과 재열람 무료</p></div><span>체험하기<Icon name="arrow" size={18}/></span></Link>
   <ConversationBanner/>
   <ReviewsPreview/>
   <section className="sg-section sg-guides"><div className="sg-heading"><h2>어떤 이야기가 필요한가요?<span>04</span></h2><Link href="/characters">전체보기<Icon name="arrow" size={16}/></Link></div><div className="sg-guide-grid">{CHARACTERS.map((c,i)=><Link href={`/characters/${c.id}`} className="sg-guide" key={c.id}><div className="sg-guide-image"><img src={CHAR_IMG[c.id]} alt={c.name} loading="lazy"/><span aria-hidden="true">{['財','命','緣','運'][i]}</span></div><div className="sg-guide-copy"><p>{subjects[i]}</p><h3>{c.name}</h3><span className="sg-guide-description">{guideReasons[i]}</span></div></Link>)}</div></section>
   <section className="sg-section sg-menus"><div className="sg-heading"><h2>내 고민에 맞는 운세</h2><button onClick={charge}>엽전 충전<Icon name="coin" size={17}/></button></div><p className="sg-group">프리미엄</p><div className="sg-paid">{MENUS.filter(m=>m.paid).map((m,i)=><Link key={m.href} href={m.href} className="sg-menu"><span className="sg-menu-number">0{i+1}</span><div><h3>{m.label}</h3><p>{m.desc}</p></div><span className="sg-menu-price">{m.href==='/gunghap'?'준비 중':<>{NYANG_PRICE.toLocaleString()}원 <small>{formatNyang(1)}</small></>}</span><Icon name="arrow" size={21}/></Link>)}</div><p className="sg-group">그 밖의 운세</p><div className="sg-free">{MENUS.filter(m=>!m.paid).map(m=><Link key={m.href} href={m.href} className="sg-menu"><Icon name={icons[m.href]} size={26}/><div><h3>{m.label}</h3><p>{m.desc}</p></div><span className="sg-menu-price">{m.badge}</span></Link>)}</div><ProductConditions/></section>
   <ResultProof/>
   <TrustNote/><ServiceFAQ/>
   {status==='unauthenticated'&&<section className="sg-welcome"><p>처음 만나는 사주궁</p><h2>처음이라면<br/>일일운세 1회 무료</h2><span>가입 계정당 한 번, 부담 없이 만나보세요.</span><button className="sg-primary" onClick={()=>openLogin("/daily")}>첫 무료 운세 시작하기<Icon name="arrow" size={20}/></button></section>}
   <section className="sg-currency"><div><Icon name="coin" size={24}/><b>{formatNyang(1)}</b><span>{NYANG_PRICE.toLocaleString()}원</span></div><p>충전하기 전,<br/>받을 결과부터 확인하세요</p><button onClick={charge}>충전하기<Icon name="arrow" size={18}/></button></section>
  ...
  <dialog className="sg-login" ref={dialog} onCancel={()=>setLogin(false)} onClose={()=>setLogin(false)} aria-labelledby="sg-login-title"><button className="sg-close" aria-label="로그인 닫기" onClick={()=>setLogin(false)}>×</button><p>처음 만나는 사주궁</p><h2 id="sg-login-title">일일운세 첫 1회 무료</h2><p>계정당 최초 1회 · 날짜가 바뀌어도 추가 지급되지 않아요.</p><div className="sg-login-options">{['kakao','google','naver'].map((provider,i)=><button key={provider} disabled={!agreed} onClick={()=>void signIn(provider,{callbackUrl:contentNoticeHref(loginTarget)})}>{['카카오','구글','네이버'][i]}로 계속하기</button>)}</div><label><input type="checkbox" checked={agreed} onChange={e=>setAgreed(e.target.checked)}/><span><Link href="/terms">이용약관</Link> 및 <Link href="/privacy">개인정보처리방침</Link>에 동의합니다 (필수)</span></label></dialog>
 </div>{showShop&&<YeopjeunShop onClose={closeShop} currentBalance={balance}/>}</>
}
