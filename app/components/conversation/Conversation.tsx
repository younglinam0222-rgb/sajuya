...
import { formatHeldNyang, formatNyang, formatNyangCost } from '@/lib/nyangDisplay';
...
function ConversationSession({ initialGuide, initialSource, authenticated }: InitialSelection & { authenticated: boolean }) {
 ...
 <div className="cn-balance"><span>첫 무료 <b>{!data ? '—' : data.freeAvailable ? '1' : '0'}/1</b></span><span>{formatHeldNyang(data?.balance || 0)}</span><button disabled={busy || loading} onClick={() => void reload()} aria-label="대화 새로 불러오기">↻</button></div>
 ...
 <footer className="cn-composer"><div className="cn-suggestions" aria-label="이어서 물어볼 질문">{suggestions.map(s => <button disabled={busy} key={s} onClick={() => { setDraft(s); pending.current = null; setError(''); input.current?.focus(); }}>{s}<span>↗</span></button>)}</div><span className="cn-connected">{data?.profiles.find(p => p.id === source)?.label || '연결할 사주를 선택해주세요'}</span><form onSubmit={e => { e.preventDefault(); requestSend(); }}><textarea ref={input} value={draft} disabled={busy} maxLength={800} rows={2} placeholder="궁금한 것을 편하게 물어보세요" aria-label="대화 질문" onChange={e => { setDraft(e.target.value); pending.current = null; setError(''); }}/><button type="submit" disabled={!draft.trim() || busy || loading || !authenticated || !source || !data?.enabled || data.remaining <= 0} aria-label="질문 보내기">↑</button></form><div className="cn-composer-meta"><span>{!data ? '이용 정보 확인 후 질문을 보낼 수 있어요.' : data.remaining === 0 ? '오늘 대화 한도에 도달했어요.' : data?.freeAvailable ? '첫 답변 무료' : `다음 답변 ${data?.cost != null ? formatNyang(data.cost) : '—'} · 전송 전 확인`}</span><span>{draft.length}/800</span></div></footer>
 </>}
 <dialog ref={pay} className="cn-pay" onCancel={closePay} aria-labelledby="cn-pay-title"><button className="cn-close" onClick={closePay} aria-label="이용 확인 닫기">×</button><span>이야기를 이어가기 전에</span><h2 id="cn-pay-title">다음 답변은 유료예요.</h2><p>첫 무료 답변을 사용했어요. 질문 1개에 대한 답변 1건을 받습니다.</p><div className="cn-pay-price"><span>답변 1건</span><b>{data?.cost != null ? formatNyang(data.cost) : '—'}</b></div><p className="cn-pay-small">{'저장된 대화 다시보기는 무료입니다. 실패한 생성의 예약 엽전은 복구합니다.'}</p>{(data?.balance || 0) < 1 ? <>{<button className="cn-primary" onClick={() => { closePay(); setShop(true); }}>엽전충전</button>}</> : <><label><input type="checkbox" checked={consent} onChange={e => setConsent(e.target.checked)}/><span>{`${formatNyang(1)} 사용 및 AI 답변 제공, 이용약관의 환불 기준을 확인했어요.`} {<Link href="/terms#refund">환불 기준</Link>}</span></label><button className="cn-primary" disabled={!consent || busy} onClick={() => void send(1)}>{`${formatNyangCost(1)} 답변 받기`}</button></>}<button className="cn-cancel" onClick={closePay}>지금은 그만하기</button></dialog>
