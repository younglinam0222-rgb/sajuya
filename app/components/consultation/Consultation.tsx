...
type RecordItem={id:string;topicId:string;characterId:string;answers:string[];note?:string;createdAt:string;result:ChatAnswer}
type Config={enabled:boolean;cost:number|null;balance:number;remaining:number;profiles:{id:string;label:string}[];records:RecordItem[];demo?:boolean}
export default function Consultation({demo=false}:{demo?:boolean}) {
 const {status}=useSession(),[config,setConfig]=useState<Config|null>(null),[tab,setTab]=useState<'chat'|'history'>('chat')
 const [guide,setGuide]=useState('baekhalma'),[group,setGroup]=useState<string>('연애·인연'),[topicId,setTopicId]=useState(''),[sourceId,setSourceId]=useState('')
 const [step,setStep]=useState(0),[answers,setAnswers]=useState<string[]>([]),[note,setNote]=useState(''),[consent,setConsent]=useState(false)
 const [busy,setBusy]=useState(false),[elapsed,setElapsed]=useState(0),[error,setError]=useState(''),[loading,setLoading]=useState(false),[result,setResult]=useState<ChatAnswer|null>(null)
 ...
 async function refresh(){setLoading(true);try{const r=await fetch('/api/chat');const data=await r.json();if(!r.ok)throw Error(data.error);setConfig(data);setSourceId(old=>data.profiles.some((p:{id:string})=>p.id===old)?old:data.profiles[0]?.id||'')}catch(e){setError(e instanceof Error?e.message:'상담 정보를 불러오지 못했어요.')}finally{setLoading(false)}}
 useEffect(()=>{if(status==='authenticated')void refresh();else if(status==='unauthenticated'){setConfig(null);setResult(null);setAnswers([]);setStep(0)}},[status])
 useEffect(()=>{const g=new URLSearchParams(window.location.search).get('guide')?.toLowerCase();if(CHAT_GUIDES.some(c=>c.id===g))setGuide(g!);},[])
 useEffect(()=>{heading.current?.focus({preventScroll:true})},[step,tab])
 useEffect(()=>{if(!busy){setElapsed(0);return}const id=setInterval(()=>setElapsed(s=>s+1),1000);return()=>clearInterval(id)},[busy])
 ...
 function previous(){if(busy)return;change();setStep(s=>Math.max(0,s-1))}
 async function submit(){
  if(lock.current||!topic||answers.length!==3||!consent)return
  ...
  try{
   ...
   if(!r.ok){const e=await r.json();throw Error(e.error||'답변을 받지 못했어요.')}
   const wire=await r.text();let text='',done=false
   for(const line of wire.split('\n'))if(line.startsWith('data: ')){const raw=line.slice(6).trim();if(raw==='[DONE]'){done=true;continue}const event=JSON.parse(raw);if(event.type==='error')throw Error('답변 생성에 실패했어요.');if(typeof event.text==='string')text+=event.text}
   if(!done)throw Error('연결이 끊겼어요. 같은 요청으로 다시 확인해주세요.')
   setResult(readChatAnswer(text));setStep(4);void refresh()
  }catch(e){setError(e instanceof Error?e.message:'답변을 불러오지 못했어요.')}finally{lock.current=false;setBusy(false)}
 }
 ...
 const canContinue=step===0?!!topic&&!!sourceId&&status==='authenticated':!!answers[step-1]
 ...
 <div className="cs-tabs" role="group" aria-label="상담 화면"><button aria-pressed={tab==='chat'} onClick={()=>setTab('chat')}>주제 선택 상담</button><button disabled={busy} aria-pressed={tab==='history'} onClick={()=>{setTab('history');void refresh()}}>내 상담 기록 {config?.records.length?`(${config.records.length})`:''}</button></div>
 <div className="cs-body">{demo&&<p className="cs-demo">비공개 체험 · 답변은 선택에 따라 구성한 예시입니다. 실제 AI 호출과 엽전 차감은 없습니다.</p>}
 {status!=='authenticated'&&<section className="cs-auth"><h2>내 사주로 이야기를 이어가요.</h2><p>{demo?'예시 계정과 사주로 상담 전체 과정을 체험해보세요.':'로그인하면 저장된 사주를 선택해 상담하고, 답변을 다시 읽을 수 있어요.'}</p>{demo?<button className="cs-primary" onClick={()=>signIn()}>예시 계정으로 체험하기</button>:<div className="cs-actions"><button onClick={()=>signIn('kakao')}>카카오로 로그인</button><button onClick={()=>signIn('naver')}>네이버로 로그인</button></div>}</section>}
 ...
 {tab==='history'?<section><h2 className="cs-title" ref={heading} tabIndex={-1}>나에게 남겨진 이야기</h2><p className="cs-muted">{demo?'이 브라우저에 저장한 체험 기록입니다.':'내 계정에 저장된 최근 상담입니다.'} 다시 읽어도 엽전을 사용하지 않아요.</p>{config?.records.length?<div className="cs-history">{config.records.map(r=><button key={r.id} onClick={()=>openRecord(r)}><span>{CHAT_GUIDES.find(g=>g.id===r.characterId)?.short} · {new Date(r.createdAt).toLocaleDateString('ko-KR')}</span><b>{CHAT_TOPICS.find(t=>t.id===r.topicId)?.title||'나의 상담'}</b><p>{r.result.summary}</p><small>상담 다시 읽기 →</small></button>)}</div>:<div className="cs-empty"><p>아직 나눈 이야기가 없어요.</p><button className="cs-primary" onClick={()=>setTab('chat')}>첫 상담 시작하기</button></div>}</section>:<>
 ...
 {step===4&&result&&<section className="cs-result"><span className="cs-result-label">{topic?.title} · 상담 답변{demo?' 예시':''}</span><h2 className="cs-title" ref={heading} tabIndex={-1}>지금 당신에게 전하는 말</h2><blockquote>{result.summary}</blockquote><details className="cs-picked"><summary>내가 나눈 이야기</summary>{answers.map(a=><p key={a}>{a}</p>)}{note&&<p>{note}</p>}</details>{[['해석',result.interpretation,'reading'],['지금 해볼 일',result.action,'action'],['마음에 두면 좋은 것',result.caution,'caution']].map(([title,body,tone])=><article key={tone} className={'cs-answer cs-'+tone}><h3>{title}</h3>{body.split('\n').filter(Boolean).map((p,i)=><p key={i}>{p}</p>)}</article>)}<aside className="cs-reflect"><span>나에게 남기는 질문</span><p>{result.nextQuestion}</p></aside><p className="cs-muted">사주 해석은 자신을 돌아보는 참고 이야기입니다. 건강·금전 문제의 진단이나 확정적인 예측을 제공하지 않습니다.</p><div className="cs-actions"><button className="cs-primary" onClick={restart}>다른 고민 상담하기</button><button onClick={()=>setTab('history')}>내 상담 기록</button></div></section>}
 {step<4&&!busy&&<div className="cs-bottom"><div className="cs-actions">{step>0&&<button onClick={previous}>이전 선택</button>}{step<3?<button className="cs-primary" disabled={!canContinue} onClick={()=>{setStep(s=>s+1);setError('')}}>다음 질문 →</button>:<button className="cs-primary" disabled={!canContinue||!consent||!config?.enabled||config.remaining<=0} onClick={submit}>{error?'같은 요청으로 다시 확인':'상담 답변 받기'}</button>}</div>{config&&<p>{demo?'샘플 체험':`보유 ${config.balance}냥`} · 오늘 남은 상담 {config.remaining}회{config.remaining<=0?' · 다음 날 다시 이용해주세요.':''}</p>}</div>}
 </>}
 {error&&<div className="cs-error" role="alert"><b>확인이 필요해요.</b><p>{error}</p><p>완료된 답변은 기록에서 먼저 확인하세요. 같은 요청의 중복 차감을 막습니다.</p><button onClick={()=>{void refresh();setTab('history')}}>상담 기록 확인</button></div>}
 {demo&&<details className="cs-demo-tools"><summary>샘플 상태 체험</summary><p>오류 뒤 다시 시도해도 체험 횟수가 중복 사용되지 않는지 확인할 수 있어요.</p><button onClick={()=>{window.dispatchEvent(new CustomEvent('chat-demo-error'));setError('다음 답변에서 오류를 한 번 체험합니다.')}}>다음 답변 오류 1회</button><button disabled={busy} onClick={()=>{window.dispatchEvent(new CustomEvent('chat-demo-reset'));restart();void refresh()}}>체험 기록·횟수 초기화</button></details>}
 </div><footer className="cs-footer">사주궁 · {demo?'체험 기록은 이 브라우저에만 저장됩니다.':'내 상담은 내 계정에서만 확인합니다.'}</footer></main>
}
