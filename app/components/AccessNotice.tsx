...
export default function AccessNotice({daily=false}:{daily?:boolean}) {
 ...
 <p>{daily?'계정당 최초 1회 무료 · 저장된 결과 다시보기 무료':'새 풀이 1회 한냥 · 동일한 입력의 저장 결과 다시보기 무료'}</p>
 {status==='loading'?null:status!=='authenticated'?<button onClick={()=>signIn(undefined,{callbackUrl:window.location.pathname+window.location.search})} className="underline">로그인하기</button>:!daily&&<button onClick={()=>setOpen(true)} className="underline">엽전충전</button>}
