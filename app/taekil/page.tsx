        ...
        <div className="max-w-md mx-auto px-4 pt-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-gray-500 text-xs">{form.targetYear}년 {form.targetMonth}월</p>
            </div>
          </div>
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white pb-24">
      <div className="max-w-md mx-auto px-4 pt-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-xl font-bold">🗓️ 택일</h1>
                ...
                {['male', 'female'].map(g => (
                  <button key={g} onClick={() => setForm(f => ({ ...f, gender: g }))}
                    className={`py-2.5 rounded-xl text-sm font-medium transition-all ${
                      form.gender === g ? 'bg-violet-500 text-white' : 'bg-gray-900 text-gray-400 border border-gray-700'
                    }`}>
                    {g === 'male' ? '남성' : '여성'}
                  </button>
                ))}
                ...
                {EVENT_TYPES.map(e => (
                  <button key={e.value} onClick={() => setForm(f => ({ ...f, eventType: e.value }))}
                    className={`py-2.5 rounded-xl text-sm font-medium transition-all flex flex-col items-center gap-1 ${
                      form.eventType === e.value ? 'bg-violet-500 text-white' : 'bg-gray-900 text-gray-400 border border-gray-700'
                    }`}>
