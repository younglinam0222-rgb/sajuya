  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const response=await fetch('/api/pay/order/'+orderId)
        const data=await response.json()
        if(!response.ok) throw new Error(data.error)
        if(data.status==='done') { router.replace('/storage'); return }
        setOrder(data)
  ...
  if (status === 'unauthenticated') {
    return (
      <div className="palace-page palace-pay min-h-screen bg-[#0a0a0f] flex flex-col items-center justify-center text-white px-6 text-center">
        ...
        <Link href={'/storage'} className="mt-6 text-xs text-gray-600">← 결과로 돌아가기</Link>
      </div>
    )
  }

  return (
    <div className="palace-page palace-pay min-h-screen bg-[#0a0a0f] text-white max-w-[430px] mx-auto pb-10">
      <div className="px-4 py-5 border-b border-gray-800 flex items-center gap-3">
        <Link href={'/storage'} className="text-gray-400 text-xl">←</Link>
        <div>
          <p className="font-bold text-base">{order?.orderName??'결제 준비'}</p>
          <p className="text-xs text-gray-500 mt-0.5">선택한 상품과 결제 금액을 확인해주세요</p>
        </div>
      </div>
      ...
      <div className="px-4 pt-5">
        <div className="rounded-2xl p-4 mb-4 bg-[#111118] border border-gray-800 flex items-center justify-between">
          ...
          <span className="text-xl font-black text-yellow-400">{(order?.amount??0).toLocaleString()}원</span>
        </div>
        ...
        <button
          ...
          {paying ? '결제 처리 중...' : !agreed ? '위 내용에 동의해주세요' : `${(order?.amount??0).toLocaleString()}원 결제하기`}
        </button>
      </div>
    </div>
  )
