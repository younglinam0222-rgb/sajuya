...
const SECTIONS = [
  ...
  { key: 'love', icon: '💕', title: '연애 궁합', color: '#F472B6' },
...
function PersonForm({
  ...
  return (
    <div className="bg-gray-900 rounded-xl p-3">
      <p className="text-xs font-bold mb-2.5" style={{ color: label === '나' ? '#EC4899' : '#8B5CF6' }}>
        {label === '나' ? '💗' : '💙'} {label}
      </p>
      <div className="space-y-2.5">
        <input
          ...
          className="w-full bg-[#0a0a0f] border border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-pink-500"
        />
        <div className="grid grid-cols-3 gap-1.5">
          <select value={form[`year${prefix}`] || '1990'}
            onChange={e => onChange(`year${prefix}`, e.target.value)}
            className="bg-[#0a0a0f] border border-gray-700 rounded-lg px-1 py-2 text-xs text-white focus:outline-none">
            {YEARS.map(y => <option key={y} value={y}>{y}년</option>)}
          </select>
          <select value={form[`month${prefix}`] || '1'}
            onChange={e => onChange(`month${prefix}`, e.target.value)}
            className="bg-[#0a0a0f] border border-gray-700 rounded-lg px-1 py-2 text-xs text-white focus:outline-none">
            {MONTHS.map(m => <option key={m} value={m}>{m}월</option>)}
          </select>
          <select value={form[`day${prefix}`] || '1'}
            onChange={e => onChange(`day${prefix}`, e.target.value)}
            className="bg-[#0a0a0f] border border-gray-700 rounded-lg px-1 py-2 text-xs text-white focus:outline-none">
        ...
        <div className="grid grid-cols-2 gap-1.5">
          {['male', 'female'].map(g => (
            <button key={g}
              onClick={() => onChange(`gender${prefix}`, g)}
              className={`py-2 rounded-lg text-xs font-medium transition-all ${
                form[`gender${prefix}`] === g
                  ? 'bg-pink-500 text-white'
                  : 'bg-[#0a0a0f] text-gray-400 border border-gray-700'
              }`}>
              {g === 'male' ? '남성' : '여성'}
            </button>
          ))}
...
export default function GunghapPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    name1: '', year1: '1990', month1: '1', day1: '1', gender1: 'female',
    name2: '', year2: '1990', month2: '1', day2: '1', gender2: 'male',
  })
        ...
        <div className="max-w-md mx-auto px-4 pt-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-gray-500 text-xs">구미호 선생의 궁합 분석</p>
            </div>
          </div>
