...
async function generate(req: NextRequest) {
  ...
  try {
    ...
    // 현재 나이 계산
    const currentYear = koreanDate().year
...
${categoryHints.includes('어울리는 지역') ? '["어울리는 지역" 카테고리 전용 지침] 이 사람의 오행 중 부족한 기운(원국에 개수가 0이거나 적은 오행)과 넘치는 기운을 먼저 구체적으로 짚고, "그래서 왜" 어떤 방향(동/서/남/북/중앙)이나 어떤 지역 특성(산 근처·물 근처·번화가·조용한 동네·높은 지대·낮은 지대 등)이 이 부족함을 채워주고 넘치는 기운을 눌러주는지 오행 상생·상극 원리로 명확하게 설명해라. "왜 그런지 이유"가 없이 결론만 던지면 실격이다. 실제 지명을 단정적으로 찍지는 말고, 이유가 딸린 방향성으로 안내해라.' : ''}
