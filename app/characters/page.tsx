...
const CHARACTERS = [
  {
    ...
    color: '#c6a66d',
    bg: 'linear-gradient(135deg, #222a34, #34362f)',
    tags: ['재물운', '직업운', '팩폭'],
    price: '1900원',
  ...
  },
  {
    ...
    color: '#8daabc',
    bg: 'linear-gradient(135deg, #182431, #2a3c4c)',
    tags: ['사주분석', '성격', '종합운세'],
    price: '1900원',
  ...
  },
  {
    ...
    color: '#bf94a4',
    bg: 'linear-gradient(135deg, #28232a, #443038)',
    tags: ['연애운', '궁합', '인연'],
    price: '1900원',
  ...
  },
  {
    ...
    color: '#94b29c',
    bg: 'linear-gradient(135deg, #192821, #2b4337)',
    tags: ['대운', '건강', '인생흐름'],
    price: '1900원',
    ...
    <div className="palace-page palace-characters min-h-screen bg-[#0c1119] text-white pb-24">
      <div className="max-w-md mx-auto px-4 pt-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="[font-family:var(--palace-serif)] text-xl font-bold">👁 운명을 보는 자들</h1>
        </div>
        <p className="text-[#a7b3c3] text-sm mb-6 ml-8">신령들의 이야기와 전문 분야</p>

        <div className="space-y-3">
          {CHARACTERS.map(c => (
            <div key={c.id}
              className="rounded-lg overflow-hidden transition-all duration-300"
              style={{ border: `1px solid ${open === c.id ? c.color : '#344151'}`, background: '#17202c' }}>
              ...
              <button
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[#a7b3c3] text-xs">{c.title}</p>
                  <div className="flex gap-1 mt-1.5 flex-wrap">
                    {c.tags.map(t => (
                      <span key={t} className="text-xs px-1.5 py-0.5 bg-[#202b39] rounded-full text-[#a7b3c3]">#{t}</span>
                    ))}
                  </div>
                </div>
                <span className="text-[#9eabbd] text-sm flex-shrink-0">{open === c.id ? '▲' : '▼'}</span>
              </button>

              {open === c.id && (
                <div className="border-t border-[#344151]/50">
                  {/* 캐릭터 큰 이미지 */}
                  <div className="h-48 relative overflow-hidden" style={{ background: c.bg }}>
                    <img src={c.img} alt={c.name}
                      className="w-full h-full object-cover object-top"
                      onError={e => { (e.target as HTMLImageElement).style.opacity = '0' }} />
                    <div className="absolute inset-0" style={{ background: 'linear-gradient(to bottom, transparent 50%, #17202c 100%)' }} />
                  </div>

                  <div className="px-4 pb-4">
                    <p className="text-[#c4cdd8] text-sm leading-relaxed mb-3">{c.desc}</p>

                    <div className="p-3 rounded-md mb-3" style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${c.color}30` }}>
                      <p className="text-xs mb-2" style={{ color: c.color }}>💬 대표 멘트</p>
                      <p className="text-[#e0e5ec] text-sm italic leading-relaxed">{c.quote}</p>
                    </div>

                    <div className="p-3 rounded-md bg-[#17202c] mb-3">
                      <p className="text-xs text-[#a7b3c3] mb-1.5">📖 뒷이야기</p>
                      <p className="text-[#c4cdd8] text-sm leading-relaxed">{c.backstory}</p>
                    </div>

                    <div className="grid grid-cols-2 gap-2 mb-3">
                      <div className="p-2.5 rounded-md bg-[#17202c]">
                        <p className="text-xs text-[#a7b3c3] mb-1">성격</p>
                        <p className="text-xs text-[#c4cdd8]">{c.personality}</p>
                      </div>
                      <div className="p-2.5 rounded-md bg-[#17202c]">
                        <p className="text-xs text-[#a7b3c3] mb-1">전문분야</p>
                        <p className="text-xs text-[#c4cdd8]">{c.specialty}</p>
                      </div>
                    </div>

                    <Link href={`/saju?character=${encodeURIComponent(c.id)}`}
                      className="block text-center py-3 rounded-md text-sm font-bold text-white"
