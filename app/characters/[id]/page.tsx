...
const CHARACTERS: Record<string, {
  name: string; title: string; emoji: string; color: string; bg: string
  tags: string[]; price: string; desc: string; backstory: string
  personality: string; specialty: string; quote: string
  sections: string[]
}> = {
  baekhalma: {
    name: '건물주 백할매', title: '부와 운명의 관리자',
    emoji: '👵', color: '#8B5CF6', bg: '#1a1025',
    tags: ['재물운', '직업운', '팩폭'],
    price: '590원',
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white pb-24">
      <div className="max-w-md mx-auto px-4 pt-6">
        ...
        {/* 캐릭터 헤더 */}
        <div className="rounded-3xl overflow-hidden mb-4" style={{ background: c.bg, border: `1px solid ${c.color}40` }}>
          <div className="p-6 text-center">
            <div className="text-7xl mb-3">{c.emoji}</div>
            <h2 className="text-2xl font-black mb-1">{c.name}</h2>
            <p className="text-sm mb-3" style={{ color: c.color }}>{c.title}</p>
        ...
        <Link href="/characters" className="block mt-3 text-center text-gray-500 text-sm">
          ← 다른 신령 보기
        </Link>
      </div>
    </div>
