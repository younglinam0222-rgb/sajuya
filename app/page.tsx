      ...
      <div className="grid grid-cols-4 text-center border-b border-gray-800">
        {pillars.map(({ label, p }) => (
          <div key={label} className="py-2" style={{ background: p ? ELEMENT_BG[p.stemElement] || '#111118' : '#111118' }}>
            <div className="text-2xl font-black" style={{ color: p ? ELEMENT_COLORS[p.stemElement] || '#fff' : '#333' }}>
              {p ? p.stem : '?'}
            </div>
            <div className="text-[9px] text-gray-500 mt-0.5">{p ? p.stemElement : ''}</div>
            <div className="text-[9px] text-gray-500">{p ? p.sipsinStem : ''}</div>
          </div>
        ))}
      </div>
      {/* 지지 */}
      <div className="grid grid-cols-4 text-center border-b border-gray-800">
        {pillars.map(({ label, p }) => (
          <div key={label} className="py-2" style={{ background: p ? ELEMENT_BG[p.branchElement] || '#111118' : '#111118' }}>
            <div className="text-2xl font-black" style={{ color: p ? ELEMENT_COLORS[p.branchElement] || '#fff' : '#333' }}>
              {p ? p.branch : '?'}
            </div>
            <div className="text-[9px] text-gray-500 mt-0.5">{p ? p.branchElement : ''}</div>
            <div className="text-[9px] text-gray-500">{p ? p.sipsinBranch : ''}</div>
          </div>
        ))}
      </div>
      {/* 오행 */}
  ...
  return (
    <div className="rounded-2xl p-4 bg-[#111118] border border-gray-800">
      <div className="flex items-center gap-2 mb-4">
        <span>📊</span><span className="font-bold text-sm text-white">나이대별 운의 흐름</span>
      </div>
      ...
      {open && (
        <div className="px-4 pb-4 border-t border-gray-800">
          <div className="mt-3 p-3 rounded-xl bg-gray-900 text-center">
            ...
            <button className="px-4 py-2 rounded-xl text-sm font-bold text-white" style={{ background: charColor }}>
              990원으로 전체 보기 →
            </button>
          </div>
        </div>
          ...
          {paidTitles.length > 0 && (
            <div className="mt-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <button className="text-xs px-3 py-1.5 rounded-full font-bold text-white" style={{ background: selectedChar.color }}>
                  990원으로 전체 열기
                </button>
              </div>
